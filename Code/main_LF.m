...
SalMapPath = '../LF_SalMap/';  

%Evaluated Models
% Models = {'CMC', 'SMFF', 'VOS', 'LVNet', 'DSG', 'HDCT', 'RCRR', 'RRWR', 'SMD', 'DSS', 'EGNet', 'PFAN', 'PoolNet', 'R3Net', 'RADF', 'DAFNet-VGG', 'DAFNet-Res'};
%  Models = { '34', '39', '49', '59', '64'};
% Models = { '16-39', '17-39'};
% Models = {'4D-Ours','2D-BASNet(CVPR2019)', '2D-CAGNet(PR2020)', '2D-CPD(CVPR2019)', '2D-ITSD(CVPR2020)', '2D-PFAN(CVPR2019)', '3D-BBSNet(ECCV2020)', '3D-CMWNet(ECCV2020)', '3D-ICNet(TIP2020)', '3D-S2MA(CVPR2020)', '3D-SSF(CVPR2020)', '4D-DLFS(IJCAI2019)', '4D-LFDCN(TIP2020)', '4D-LFNet(TIP2020)', '4D-MoLF(NeurIPS2019)', '4D-MTCNet(TCSVT2020)'};
% Models = {'4D-Ours', '3D-S2MA(CVPR2020)', '3D-SSF(CVPR2020)', '4D-DLFS(IJCAI2019)', '4D-LFDCN(TIP2020)', '4D-LFNet(TIP2020)', '4D-MoLF(NeurIPS2019)', '4D-MTCNet(TCSVT2020)'};
Models = { 'HAINet'};
...
Datasets = {'HFUT-Lytro Illum(TIP2020)', 'HFUT-Lytro(TOMCCAP2017)'};
% Datasets = {'HFUT-Lytro(TOMCCAP2017)'};
% Datasets = {'ORSSD'};

%%

%Evaluated Score Results
% ResDir = '../Result_image_sod/';
...
%         fprintf(fileID, '(Dataset:%s; Model:%s) Smeasure:%.4f; MAE:%.4f; adpEm:%.4f; meanEm:%.4f; maxEm:%.4f; adpFm:%.4f; meanFm:%.4f; maxFm:%.4f.\n',dataset,model,Smeasure, mae, adpEm, meanEm, maxEm, adpFm, meanFm, maxFm);   
        save([ResPath model],'Smeasure', 'mae', 'WeiFm', 'column_Pr', 'column_Rec', 'column_F', 'adpFm', 'meanFm', 'maxFm', 'column_E', 'adpEm', 'meanEm', 'maxEm');
        fprintf(fileID, '(Dataset:%s; Model:%s) Smeasure:%.4f; MAE:%.4f; WeiFm:%.4f; adpEm:%.4f; meanEm:%.4f; maxEm:%.4f; adpFm:%.4f; meanFm:%.4f; maxFm:%.4f.\n',dataset,model,Smeasure, mae, WeiFm, adpEm, meanEm, maxEm, adpFm, meanFm, maxFm);       
